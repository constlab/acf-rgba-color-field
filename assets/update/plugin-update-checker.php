...
class PluginUpdateChecker_2_1 {
	...
	public function requestInfo($queryArgs = array()){
		...
		if ( !is_wp_error($result) && isset($result['response']['code']) && ($result['response']['code'] == 200) && !empty($result['body']) ){
			...
			$message = sprintf(__("The URL %s does not point to a valid plugin metadata file. ", 'dhz_updater'), $url);
			if ( is_wp_error($result) ) {
				$message .= __("WP HTTP error: ", 'dhz_updater') . $result->get_error_message();
			} else if ( isset($result['response']['code']) ) {
				$message .= __("HTTP response code is ", 'dhz_updater') . $result['response']['code'] . __(" (expected: 200)", 'dhz_updater');
			} else {
				$message .= __("wp_remote_get() returned an unexpected result.", 'dhz_updater');
			}
	...
	public function getInstalledVersion(){
		...
		if ( isset($pluginHeader['Version']) ) {
			...
			if ( $this->debugMode ) {
				trigger_error(
					sprintf(
						__("Can't to read the Version header for '%s'. The filename is incorrect or is not a plugin.", 'dhz_updater'),
						$this->pluginFile
					),
					E_USER_WARNING
				);
	...
	protected function getPluginHeader() {
		if ( !is_file($this->pluginAbsolutePath) ) {
			...
			if ( $this->debugMode ) {
				trigger_error(
					sprintf(
						__("Can't to read the plugin header for '%s'. The file does not exist.", 'dhz_updater'),
						$this->pluginFile
					),
					E_USER_WARNING
				);
	...
	public function checkForUpdates(){
		...
		if ( $installedVersion === null ) {
			if ( $this->debugMode ) {
				trigger_error(
					sprintf(__('Skipping update check for %s - installed version unknown.', 'dhz_updater'), $this->pluginFile),
					E_USER_WARNING
				);
			}
			return null;
	...
	function fixDirectoryName($source, $remoteSource, $upgrader) {
		...
		if ( $source !== $correctedSource ) {
			...
			if ( is_array($sourceFiles) ) {
				...
				if ( (count($sourceFiles) > 1) || (!$wp_filesystem->is_dir($firstFilePath)) ) {
					return new WP_Error(
						...
						sprintf(
							__('The directory structure of the update is incorrect. All plugin files should be inside ', 'dhz_updater') .
							__('a directory named <span class="code">%s</span>, not at the root of the ZIP file.', 'dhz_updater'),
			...
			$upgrader->skin->feedback(sprintf(
				__('Renaming %s to %s&#8230;', 'dhz_updater'),
				'<span class="code">' . basename($source) . '</span>',
				'<span class="code">' . $pluginDirectoryName . '</span>'
			));

			if ( $wp_filesystem->move($source, $correctedSource, true) ) {
				$upgrader->skin->feedback(__('Plugin directory successfully renamed.', 'dhz_updater'));
				return $correctedSource;
			} else {
				return new WP_Error(
					'puc-rename-failed',
					__('Unable to rename the update to match the existing plugin directory.', 'dhz_updater')
				);
			}
		}
	...
	public function addCheckForUpdatesLink($pluginMeta, $pluginFile) {
		...
		if ( $isRelevant && current_user_can('update_plugins') ) {
			...
			$linkText = apply_filters('puc_manual_check_link-' . $this->slug, __('Check for updates', 'dhz_updater') );
			if ( !empty($linkText) ) {
	...
	public function displayManualCheckResult() {
		if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->slug) ) {
			...
			if ( $status == 'no_update' ) {
				$message = __('This plugin is up to date.', 'dhz_updater');
			} else if ( $status == 'update_available' ) {
				$message = __('A new version of this plugin is available.', 'dhz_updater');
			} else {
				$message = sprintf(__('Unknown update checker status "%s"', 'dhz_updater'), htmlentities($status));
			}
			printf(
...
class PluginInfo_2_1 {
	...
	public static function fromJson($json, $triggerErrors = false){
		...
		if ( empty($apiResponse) || !is_object($apiResponse) ){
			if ( $triggerErrors ) {
				trigger_error(
					__("Failed to parse plugin metadata. Try validating your .json file with http://jsonlint.com/", 'dhz_updater'),
					E_USER_NOTICE
				);
			}
			return null;
		...
		if ( !$valid ){
			if ( $triggerErrors ) {
				trigger_error(
					__("The plugin metadata file does not contain the required 'name' and/or 'version' keys.", 'dhz_updater'),
					E_USER_NOTICE
				);
			}
			return null;
